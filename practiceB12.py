...
for values in events:
    total_item_price +=values["item_price"]
    if values["timestamp"] < min_timestamp:
        min_timestamp = values["timestamp"]
        main_sessionId = values["sessionId"]
    elif values["firstInSession"] == True:
        main_pageviewid = values["pageViewId"]
    for key, options in values.items():
        if options == "itemViewEvent":
            total_viewEvent +=1
        elif options == "itemBuyEvent":
            total_buyEvent +=1
        elif key == "detectedDuplicate":
            if options == True:
                total_duplicate +=1

print("ItemViewEvent: ", total_viewEvent)
print("ItemBuyEvent: ", total_buyEvent)
print(total_duplicate)
print(min_timestamp)
print(total_item_price)
print(main_sessionId)
print(main_pageviewid)